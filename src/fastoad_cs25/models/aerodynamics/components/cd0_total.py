...
class Cd0Total(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        if self.options["low_speed_aero"]:
            ...
            self.add_input(
                "data:aerodynamics:horizontal_tail:low_speed:CD0", units="unitless", val=np.nan
            )
            self.add_input(
                "data:aerodynamics:vertical_tail:low_speed:CD0", units="unitless", val=np.nan
            )
            self.add_input("data:aerodynamics:nacelles:low_speed:CD0", units="unitless", val=np.nan)
            self.add_input("data:aerodynamics:pylons:low_speed:CD0", units="unitless", val=np.nan)
            self.add_output(
            ...
            self.add_input(
                "data:aerodynamics:horizontal_tail:cruise:CD0", units="unitless", val=np.nan
            )
            self.add_input(
                "data:aerodynamics:vertical_tail:cruise:CD0", units="unitless", val=np.nan
            )
            self.add_input("data:aerodynamics:nacelles:cruise:CD0", units="unitless", val=np.nan)
            self.add_input("data:aerodynamics:pylons:cruise:CD0", units="unitless", val=np.nan)
            self.add_output(
