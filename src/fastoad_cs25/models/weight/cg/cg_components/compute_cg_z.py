...
from ..constants import SERVICE_EMPTY_AIRCRAFT_CG_Z


@oad.RegisterSubmodel(
    SERVICE_EMPTY_AIRCRAFT_CG_Z, "fastoad.submodel.weight.cg.empty_aircraft.z.legacy"
)
class ComputeCGZ(om.ExplicitComponent):
    def initialize(self):
        self.options.declare(
            ...
            "CG in the z-axis. Items' names will be appended with 'CG:z' for the position of "
            "the item's CG and 'mass' for the item's mass.",
        )

    def setup(self):
        for item_names in self.options["cg_z_item_names"]:
            self.add_input(item_names + "CG:z", val=np.nan, units="m")
    ...
    def setup_partials(self):
        for item_names in self.options["cg_z_item_names"]:
            self.declare_partials(
                "data:weight:aircraft_empty:CG:z", item_names + "mass", method="exact"
            )
            self.declare_partials(
                "data:weight:aircraft_empty:CG:z", item_names + "CG:z", method="exact"
            )
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        for item_names in self.options["cg_z_item_names"]:
            total_moment += inputs[item_names + "CG:z"] * inputs[item_names + "mass"]
            total_mass += inputs[item_names + "mass"]

        outputs["data:weight:aircraft_empty:CG:z"] = total_moment / total_mass
    ...
    def compute_partials(self, inputs, partials, discrete_inputs=None):
        ...
        for item_names in self.options["cg_z_item_names"]:
            partials["data:weight:aircraft_empty:CG:z", item_names + "CG:z"] = (
                inputs[item_names + "mass"] / total_mass
            )
            partials["data:weight:aircraft_empty:CG:z", item_names + "mass"] = (
                inputs[item_names + "CG:z"] * total_mass - total_moment
            ) / total_mass**2.0
